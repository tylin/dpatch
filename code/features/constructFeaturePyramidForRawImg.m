...
for i = 1 : length(levels)
  ...
  [nrows, ncols, ~] = size(feat);
  feat=cat(3,feat(:,:,1:31),imresize(im2(:,:,2),[nrows ncols],'bilinear'),imresize(im2(:,:,3),[nrows ncols],'bilinear'));  % add a and b color chanel by resizing it
  pyramidLevs{lev} = feat(:, :, 1:33);
end
canoSize.nrows = rows;
canoSize.ncols = cols;
