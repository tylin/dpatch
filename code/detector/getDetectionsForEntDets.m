...
detections = struct('features', [], 'metadata', [], 'decision', [], ...
  'totalProcessed', totalProcessed, 'thresh', [], 'heatmap', []);
...
for j = 1 : size(decision, 2)
    ind_cur = 1;
    heatmap = cell(1, length(pyramid.scales));
    for i = 1:length(pyramid.scales)  
        im_size = size(pyramid.features{i});
        len = im_size(1)*im_size(2);
        level_ind = find(levels == i);
        heatmap{i} = reshape(decision(level_ind, j), indexes(level_ind(end),:) );
        ind_cur = ind_cur + len;
    end
    detections(j).heatmap = heatmap;
end
